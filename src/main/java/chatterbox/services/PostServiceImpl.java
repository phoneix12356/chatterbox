...
public class PostServiceImpl implements PostService {
    ...
    public void createPost(CreatePostRequestDto createPostRequestDto) {
        CustomUserDetail userDetail = (CustomUserDetail) SecurityContextHolder.getContext().getAuthentication()
                .getPrincipal();
        Users foundUser = userRepository.findById(userDetail.getId())
                .orElseThrow(() -> new UsernameNotFoundException("User not found who did these post"));
        Post post = new Post();
    ...
    public GetPostByIdDto getPost(Long id) {
        ...
        GetPostByIdDto dto = modelMapper.map(post, GetPostByIdDto.class);
    ...
    public List<GetPostByIdDto> getAllThePostOfALLUser(int pageNumber, int pageSize) {
        ...
        List<GetPostByIdDto> responseDto = postList.stream().map(post -> {
            GetPostByIdDto dto = this.modelMapper.map(post, GetPostByIdDto.class);
            if (post.getUser() != null) {
                dto.setUserName(post.getUser().getUsername());
                dto.setUserAvatar(post.getUser().getAvatar());
            }
    ...
    public List<GetPostByIdDto> getAllThePostParticularUser(int pageNumber, int pageSize, Long postId) {
        ...
        List<GetPostByIdDto> lst = repository.findPostsByUserIdUsingPagination(postId, pageSize, pageNumber * pageSize)
                .stream().map((post) -> {
                    GetPostByIdDto dto = this.modelMapper.map(post, GetPostByIdDto.class);
                    if (post.getUser() != null) {
                        dto.setUserName(post.getUser().getUsername());
